...
window.onload = function() {
  ...
  if(val[1] == '1'){
    ...
    // }
     var rating = Math.round((q1+q2+q3+q4+q5+q6+q7+q8+q9+q10)/10);
    if(count == 0){
      rating = 0;
    }
    // console.log("---------------");
  ...
  fetch(" http://localhost:8080/getEmployerFeedback")
...
function parseDataforIndividualResponse(data){
  for(let key in data){
    ...
    var textKeys = ["employeeId","addressOfCompany","companyName","dept","employeeDesignation","employeeName","evaluatingPersonName"];
...
function searchbyRollNo(){
  ...
  var url = "http://localhost:8080/getEmployerFeedbackbysearch?regno="+rollno+"";
  fetch(url)
